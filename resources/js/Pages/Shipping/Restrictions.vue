<template>
    <div class="min-h-screen bg-slate-50 p-6 md:p-12">
        <div class="max-w-7xl mx-auto">
            <div
                class="flex flex-col md:flex-row items-center justify-between mb-8 p-6 bg-white rounded-2xl shadow-sm border border-slate-200 gap-4">
            ...
            <div class="grid md:grid-cols-2 gap-8 mb-12">
                <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="bg-emerald-50 border-b border-emerald-100 p-5 font-extrabold text-emerald-900 text-sm ">✓
                        Allowed Countries</div>
                    <div class="p-6">
                        <div class="relative mb-6">
                            ...
                            <button @click="add('allowed')"
                                class="absolute right-3 top-2 bg-emerald-500 p-2 rounded-lg text-white">+</button>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <div v-for="(c, i) in form.allowed" :key="i"
                                class="flex items-center gap-2 bg-white px-3 py-1 rounded-lg border border-slate-200 font-bold text-xs  shadow-sm">
                                <span>{{c}}</span>
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="bg-rose-50 border-b border-rose-100 p-5 font-extrabold text-rose-900 text-sm ">✕ Excluded
                        Countries</div>
                    <div class="p-6">
                        <div class="relative mb-6">
                            <input v-model="newExcluded" @keyup.enter="add('excluded')" placeholder="ADD ISO (e.g. CA)"
                                class="w-full pl-4 pr-12 py-3 bg-slate-50 border-2 rounded-2xl outline-none font-bold ">
                            <button @click="add('excluded')"
                                class="absolute right-3 top-2 bg-rose-500 p-2 rounded-lg text-white">+</button>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <div v-for="(c, i) in form.excluded" :key="i"
                                class="flex items-center gap-2 bg-white px-3 py-1 rounded-lg border border-slate-200 font-bold text-xs  shadow-sm">
                                <span>{{c}}</span>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                    ...
                    <button @click="exportToExcel"
                        class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-xl text-xs font-bold transition-all shadow-md flex items-center gap-2">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="log in logs" :key="log.id" class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="p-5"><span
                                        class="bg-slate-900 text-white px-2 py-1 rounded text-[10px] font-bold">{{
                                        log.country }}</span></td>
...
/**
 * Add country to allowed / excluded list
 */
const add = (type) => {
    ...
    if (form[type].includes(val)) {
        Swal.fire('Already Added', `${val} is already in the list.`, 'info');
        field.value = '';
        return;
    }
 ...
 */
const exportToExcel = () => {
    let csv = "\uFEFFORDER,COUNTRY,ALLOWED RULES,EXCLUDED RULES,STATUS,DATE\r\n";

    props.logs.forEach(log => {
        csv += `"#${log.id}","${log.country}","${log.allowed}","${log.excluded}","${log.status.trim()}","${log.date.split(' ')[0]}"\r\n`;
    });
